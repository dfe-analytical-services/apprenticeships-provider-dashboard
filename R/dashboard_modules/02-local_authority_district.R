...
lad_boundaries_2025 <- sf::st_read(
  "data/boundary_files/Local_Authority_Districts_May_2024_Boundaries_UK_BUC_-3799209068982948111.gpkg",
  quiet = TRUE
) |> # nolint: line-length-linter
  rename("lad_name" = LAD24NM)
lad_boundaries_2024 <- sf::st_read(
  "data/boundary_files/Local_Authority_Districts_May_2024_Boundaries_UK_BUC_-3799209068982948111.gpkg",
  quiet = TRUE
) |> # nolint: line-length-linter
  rename("lad_name" = LAD24NM)
lad_boundaries_2023 <- sf::st_read(
  "data/boundary_files/Local_Authority_Districts_May_2023_UK_BUC_V2_8757178717458576320.gpkg",
  quiet = TRUE
) |> # nolint: line-length-linter
  rename("lad_name" = LAD23NM)
lad_boundaries_2022 <- sf::st_read(
  "data/boundary_files/Local_Authority_Districts_December_2022_UK_BUC_V2_3956567894081366924.gpkg",
  quiet = TRUE
) |> # nolint: line-length-linter
...
delivery_lad_choices <- c(
  "",
  distinct(lad_map_parquet, `Delivery LAD`) |> pull()
)
delivery_lad_choices <- sort(delivery_lad_choices)

learner_home_lad_choices <- c(
  "",
  distinct(lad_map_parquet, `Learner home LAD`) |> pull()
)
      ...
      navset_card_tab(
        ...
        nav_panel(
          ...
          shinyGovstyle::radio_button_Input(
            ...
            choices = c("CSV (Up to 19.96 MB)", "XLSX (Up to 6.22 MB)"),
            selected = "CSV (Up to 19.96 MB)"
          ),
          downloadButton(
            NS(id, "download_data"),
            label = "Download data",
...
lad_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    updateSelectizeInput(
      session,
      "delivery_lad",
      choices = delivery_lad_choices,
      server = TRUE
    )
    updateSelectizeInput(
      session,
      "learner_home_lad",
      choices = learner_home_lad_choices,
      server = TRUE
    )
    ...
    observeEvent(input$delivery_lad_map_shape_click, {
      ...
      updateSelectizeInput(
        session,
        "delivery_lad",
        selected = map_selected_delivery_lad$id
      )
    })
    ...
    observeEvent(input$learner_home_lad_map_shape_click, {
      ...
      updateSelectizeInput(
        session,
        "learner_home_lad",
        selected = map_selected_learner_home_lad$id
      )
    })
    ...
    prov_selection_table <- reactive({
      ...
      if (input$delivery_lad != "") {
        prov_selection_table <- prov_selection_table |>
          filter(`Delivery LAD` == input$delivery_lad)
      }

      # Filter based on learner home LAD if selected
      if (input$learner_home_lad != "") {
        prov_selection_table <- prov_selection_table |>
          filter(`Learner home LAD` == input$learner_home_lad)
      }
      ...
      prov_selection_table <- prov_selection_table |>
        with_groups(
          "provider_name",
          summarise,
          `Number of apprenticeships` = sum(
            !!sym(firstlow(input$measure)),
            na.rm = TRUE
          )
        ) |>
    ...
    output$learner_home_lad_table <- renderReactable({
      validate(need(
        nrow(learner_home_lad_table()) > 0,
        paste0("No ", input$measure, " for these selections.")
      ))
    ...
    delivery_map_data <- reactive({
      ...
      boundary_data() |>
        right_join(
          delivery_lad_table(),
          by = join_by("lad_name" == "Delivery LAD")
        ) |>
    ...
    learner_home_map_data <- reactive({
      ...
      boundary_data() |>
        right_join(
          learner_home_lad_table(),
          by = join_by("lad_name" == "Learner home LAD")
        ) |>
        sf::st_transform(crs = 4326) # transform coordinates to a system we can use in leaflet maps in the app
    })
    ...
    output$learner_home_lad_map <- renderLeaflet({
      ...
      dfe_lad_map(
        learner_home_map_data(),
        input$measure,
        NS(id, "learner_home_lad")
      )
    })
    ...
    output$download_data <- downloadHandler(
      ...
      filename = function(name) {
        ...
        extension <- if (input$file_type == "CSV (Up to 19.96 MB)") {
          ".csv"
        } else {
          ".xlsx"
        }
      ...
      content = function(file) {
        if (input$file_type == "CSV (Up to 19.96 MB)" & input$provider == "") {
          ...
          input$file_type == "CSV (Up to 19.96 MB)" & input$provider != ""
        ) {
          data.table::fwrite(
            map_data() |>
              filter(year %in% input$year) |>
              filter(provider_name %in% input$provider),
            file
          )
        } else if (
          input$file_type == "XLSX (Up to 6.22 KB)" & input$provider != ""
        ) {
