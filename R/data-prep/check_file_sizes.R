...
if (!is.null(sas_parquet)) {
  ...
  data.table::fwrite(sas_parquet %>% filter(year == "2021/22") %>% collect(), "data/test.csv")
  ...
  data.table::fwrite(sas_parquet %>% filter(year == "2023/24 (Q3 Aug to Apr)") %>% collect(), "data/test.csv")
